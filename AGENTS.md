...
**Research Summary (2026-07-10)**: See CURRENT_RESEARCH.md for the full report. Headlines:
- **CAGE-1 (arXiv:2607.03510v1, Jul 2026)**: enterprise agentic AI governance evaluation framework. Action handling states: *admitted, held, narrowed, refused, escalated, quarantined, made non-effective*. Maps 1:1 onto our `CrossCheckOutcome` (ALLOW, HOLD_PENDING_*, REJECT). `HOLD_PENDING_CHAIN` is CAGE-1's "held (narrowed)". The substrate is now CAGE-1-shaped.
- **JADEPUFFER (Sysdig Threat Research, late June 2026 — disclosed Jul 8-10)**: first documented end-to-end **agentic ransomware** operation. CVE-2025-3248 (Langflow unauth RCE, CVSS 9.8) → autonomous LLM-driven recon → credential harvest (Langflow Postgres + MinIO) → lateral move (CVE-2021-29441 Nacos) → encryption of 1,342 Nacos config items → ransom note. The agent adapted in real time (fixed a failed login in 31 seconds, rewrote parsers when MinIO returned XML). Our internal threat-model term (JADEPUFFER) matches the live attack's threat-actor name. The attack chain is: `read_env → http_get_external → read_secret_store → http_post_external` — exactly the pattern our gate's `secret_to_external` + `write_after_untrusted` + `too_many_distinct_sinks` deny reasons catch.
- **Forbes "Agent Gateways Are Becoming the Control Plane" (Jul 5, 2026)**: Nutanix shipped the "Nutanix Agent Gateway" as part of Enterprise AI 2.7. Our `GovernedActionLoop` is a self-hosted, open agent gateway with 6 substrates, an audit log, and a real-world-validated threat model.
- **NVIDIA ASPIRE (Jul 3, 2026)**: Agentic Skill Programming through Iterative Robot Exploration — writes/​refines robot control programs, distills validated fixes into a reusable skill library. A central coordinator dispatches actor coding agents. The "validated fix → reusable skill library" pattern is the same distillation idea the substrate's skill acquisition would benefit from (chain verdicts that fire ALLOW repeatedly = positive corpus; new policies proposed → validated → accepted/rejected).
- **CAGE-1's Prebind Assurance** concept: "proving an agentic action is controlled before it becomes binding or operational." This is exactly what `GovernedActionLoop.propose()` does — the proposal is *proposed* until all 6 substrates agree; only then does the certificate become ACTIVE.

**Build Task: Wire `CompositionalPolicyGate` into `GovernedActionLoop.run()` (Step 6)** ⭐ today's primary build

**Motivation**: The 2026-07-09 morning's `CompositionalPolicyGate` was a *standalone* substrate: a chain could be evaluated in isolation, but the agent loop didn't consult it on the proposed action. The next-priority item on the morning + afternoon builds was explicit: "Wire `CompositionalPolicyGate` into `GovernedActionLoop.run()` — every act → observe cycle pre-checks the proposed chain. The verb bundle is the per-verb policy; the compositional gate is the chain policy. The substrate's agent loop then has both." Today's run closes that carryover.

**Key Components**:

1. **`CrossCheckOutcome.HOLD_PENDING_CHAIN`** — the 7th outcome in the enum. Mirrors the gate's `ALLOW_ONLY_REVIEW` action. Distinct from `HOLD_PENDING_HUMAN`, `HOLD_PENDING_EVIDENCE`, `HOLD_PENDING_RING`, `HOLD_PENDING_CEF`. A downstream UI can render "operator-visible hold for chain review" specifically.
2. **`CrossCheckReport` chain fields** — `chain_verdict: Optional[Dict[str, Any]] = None`, `chain_reasons: Tuple[str, ...] = ()`, `chain_digest: str = ""`, `chain_audit_index: int = -1`. All preserved on `to_dict()`. The digest is a content-addressed hash of the verdict payload; the audit index points to the record in the gate's `audit_log` list. Together, these let a downstream replay layer reconstruct the chain's decision from the gate's audit alone.
3. **`GovernedActionRequest.planned_chain: Optional[List[ChainStep]] = None`** — when supplied, the gate evaluates the chain. When absent, the chain check is silent (backward compatible: every existing call site keeps working).
4. **`GovernedActionLoop.__init__` accepts `compositional_gate: Optional[CompositionalPolicyGate] = None`** — a gate is optional. If not attached, the chain-substrate step is skipped. This is the same pattern as the existing CEF detector (also optional, also a soft substrate).
5. **Step 6 chain check in `propose()`** — runs *after* the CEF detector (Step 5) and *after* the breaker (Step 2). Conservative posture:
   - `ALLOW` → cross-check continues to the final accept block; chain verdict recorded on the report.
   - `ALLOW_ONLY_REVIEW` → `HOLD_PENDING_CHAIN` (operator-visible hold for chain review).
   - `BLOCK_AND_ESCALATE` → `REJECT` (hard ceiling; the chain is an active attack pattern).
6. **The chain verdict's `digest` + `audit_index` are preserved on the report** for replay. A future drift-signal layer can re-walk the gate's audit log and detect benign-config / chain-topology drift.

**Conservative posture**:
- Chain check is **silent** when the gate is not attached. Backward compatibility: every existing call site keeps working.
- Chain check is **silent** when `request.planned_chain` is `None`. The substrate doesn't force callers to think in chains.
- The chain verdict's `digest` is a content-addressed hash. The audit index points to the record in the gate's `audit_log` list.
- The chain gate **never mutates** the gate itself. The substrate is read-only. This matches the broader pattern (CEF detector, VerbPolicyBundle, ConstraintPressureProbe, ProbabilisticTripEngine are all read-only).
...
- Step 6 runs *after* Steps 1-5. The earliest decisive substrate short-circuits: bridge REJECT, breaker CRITICAL, ledger CONTRADICTED, ring refused, CEF CRITICAL — all of these return *before* the chain gate is consulted. The substrate fails at the *earliest* decisive substrate, not the last.
- Step 6 is *non-mutating* on the gate. The gate's `audit_log` grows; the gate's policies do not change. A `verify_chain()` method (carried forward) is the substrate for a future drift-signal layer.
...
- `test_chain_check_runs_after_breaker_critical` — **corrected during test pass**: breaker CRITICAL short-circuits at Step 2; chain gate not consulted. The original test asserted the wrong invariant; corrected to match the substrate's conservative posture (substrate fails at the earliest decisive substrate).
...
**Real findings the test pass surfaced**:
...
2. **The `chain_digest` is non-empty even on clean chains.** Because the gate's `check_chain()` always produces a verdict (even on ALLOW), the digest is always populated when the gate is attached AND a chain is supplied.
3. **`HOLD_PENDING_CHAIN` is distinct from `HOLD_PENDING_CEF` and the other 4 hold types.** The test set exercises this: `test_secret_to_external_holds_for_chain_review` produces REJECT (not HOLD), and `test_chain_with_too_many_sinks_holds` produces HOLD_PENDING_CHAIN (not HOLD_PENDING_HUMAN).
...
- `CURRENT_RESEARCH.md` — appended 2026-07-10 entry with research + build synthesis.
- `BUILD_LOG_2026-07-10.md` — this build log.
- `AGENTS.md` — this prepended entry.

**End-to-end demo**:
- JADEPUFFER chain (`read_env → http_post_external → read_secret_store → http_post_external`) on a gate with `is_secret_emitting=True` policies → REJECT, `secret_to_external` in reasons, cert REJECTED, `chain_verdict["action"] = "block_and_escalate"`.
...
- CEF CRITICAL (CET) + planned chain → HOLD_PENDING_HUMAN, chain gate not consulted.
- Bridge REJECT (strict externality) + planned chain → REJECT, chain gate not consulted.

**Research synthesis**:
- **The substrate is now CAGE-1-shaped.** The cross-check's outcome vocabulary (admitted, held, narrowed, refused, escalated, quarantined, made non-effective) maps 1:1 onto CAGE-1's vocabulary. An operator can now produce a CAGE-1 evaluation report from the substrate's audit logs: "admitted: 0; held for evidence: 3; held for ring: 1; held for CEF: 2; held for chain: 5; held for human: 4; refused: 2."
- **The JADEPUFFER real-world attack is the live validation of the substrate's threat model.** The attack chain (Langflow CVE → recon → credential harvest → lateral move → encrypt → ransom note) maps onto the gate's deny reasons: `secret_to_external` (credential + exfil), `too_many_distinct_sinks` (multi-service fanout), `write_after_untrusted` (Langflow CVE entry is the untrusted step). The fact that our internal threat-model term matches the real-world threat-actor name is a marker that the substrate was tracking the right attack pattern *before* the live attack was disclosed.
...
- **The substrate is now the *enterprise agent gateway* that Forbes described (Jul 5, 2026).** Nutanix shipped the "Nutanix Agent Gateway" as part of Enterprise AI 2.7. Our `GovernedActionLoop` is a self-hosted, open agent gateway with 6 substrates, an audit log, and a real-world-validated threat model.
...
- **DSCC clearance-mode wiring** — partition the policy registry into classification-level clusters; pre-compute the MRS (Most Restrictive Set) for each cluster. arXiv:2607.03423 reports 79.2% / 95.5% block rate with this approach.
- **CONTRA-style benign-config finder** — a tree-search that searches the policy space for benign-looking configs that produce dangerous chains. The JADEPUFFER attack is the search's *target*.
- **NVIDIA ASPIRE-style skill distillation** — successful chain verdicts become a positive-corpus; new policies are proposed by the skill-acquisition module, validated by the gate, and either accepted (added to the registry) or rejected (logged for review).
- **CAGE-1 evaluation report CLI** — `python -m cli.cage1_report` reads the loop's `reports` list and emits a CAGE-1-shaped evaluation.
...
- **Per-verb emergence profile across compositional policies** — same Probe harness as CPP, but the constraint source is the policy registry. The probe is the empirical instrument for the policy's safety claim.
...
**Research Summary (July 6, 2026)**: See CURRENT_RESEARCH.md for the full report. Headlines:
- **arXiv:2606.19390v1 (Execution-bound advisory automation for agentic AI: a reproducible AIBOM-driven CSAF-VEX framework)**: "advisories are produced from combined static and runtime evidence, cryptographically signed, and verifiable through deterministic replay". Our advisory format borrows CSAF-VEX's skeleton (document tracking + vulnerabilities[] + product_tree) but stays operator-friendly JSON. The audit log IS the runtime evidence; the bundle snapshot is the static evidence.
- **AIBOM-as-SBOM-for-AI (Security Magazine, IST policy memo, Safeguard 2026)**: AI bill of materials is the auditable, machine-readable declaration of model identity + provenance + dependencies + runtime stack + guardrails. The advisory emitter reads the bundle snapshot + audit log and emits a single self-contained JSON with all four.
- **MedSkillAudit (AIPOCH, 2026-06-29)**: a "pre-deployment audit framework" with a "two-stage evaluation" (static 40% + dynamic 60%). The advisory exposes the same two-stage structure: static (bundle snapshot) + dynamic (CPP audit log).
- **Agent Gateway control plane (Forbes Jul 5; Nutanix; Arcade on Azure/AWS marketplaces)**: the advisory is the "tamper-evident audit" the gateway emits when the control plane triggers a response. The advisory carries the governance signal downstream.
- **AIVEX (Cybernoz, Jul 1 2026)**: AI-exploitation-aware VEX extension with SRIL (Safety Relevance Interpretation Layer) for machine-readable triage decisions. Aligns with our category hierarchy (THANATOSIS > POLICY_LOOSENING > CEF_EMERGENCE > GENERIC) and recommendation ladder (BLOCK > REMEDIATE > MONITOR > DEFER).

**Build Task: AIBOM Advisory Emitter (`core/aibom_advisory.py` + `cli/aibom_review.py`)**

**Motivation (converging signals)**:
1. **2026-07-05 carryover**: The CPP build's next-priority was "AIBOM advisory emitter (`cli/cpp_review.py`) — reads a CPP JSONL audit log + a bundle snapshot and emits a CSAF-VEX-shaped advisory document. The bridge from probe audit log to 'execution-bound advisory' per arXiv:2606.19390." Today's build closes that carryover.
2. **arXiv:2606.19390 (Execution-bound advisory automation)**: the formal reference for the AIBOM/CSAF-VEX framework. The advisory emitter implements the central claim: "advisories are produced from combined static and runtime evidence, cryptographically signed, and verifiable through deterministic replay". Our advisory is *signature-ready* (content-addressed advisory_id = sha256 of canonical evidence) but unsigned today — the next-priority item.
3. **AIBOM-as-SBOM-for-AI (Security Magazine)**: the advisory's `product_tree.components[]` is the AIBOM surface — one component per (verb_name, verb_version) pair, with policy provenance + observed exploitability + status. This is the per-tool AIBOM that an agent gateway's OPA evaluator reads.
4. **MedSkillAudit (AIPOCH)**: the two-stage pre-deployment audit shape is exact — `bundle_snapshot` is the static 40%, `audit_log_records` is the dynamic 60%. The advisory's `document.bundle_summary` + `vulnerabilities[]` are the static + dynamic surfaces. The advisory is the *replay verifier* for the audit.
5. **Agent Gateway control plane (Forbes + Nutanix + Arcade)**: the advisory is the "tamper-evident audit log" that the gateway emits on policy violation. The `evidence_digest` is the hash that the gateway's signature scheme attaches to. The recommendation field (block/remediate/monitor/defer) is what the gateway's OPA policy evaluator consumes.

**Key components**:

1. **`AdvisoryCategory` (str-enum)** — `GENERIC / CEF_EMERGENCE / POLICY_LOOSENING / THANATOSIS / BUNDLE_GAP`. Priority order: THANATOSIS > POLICY_LOOSENING > CEF_EMERGENCE > GENERIC. CET (simulated_crash+critical) wins over policy_loosening wins over plain CEF emergence wins over generic.
2. **`AdvisorySeverity` (str-enum)** — `NONE / LOW / MEDIUM / HIGH / CRITICAL`. Mirrors CEFSeverity; `rank()` helper for ordering.
3. **`AdvisoryAction` (str-enum)** — `DEFER / MONITOR / REMEDIATE / BLOCK`. Maps loosely to the CSAF-VEX product_status field; "block" is the new entry; "defer" + "monitor" are the "not_affected" / "fixed" continuums from VEX.
4. **`AdvisoryStatus` (str-enum)** — `DRAFT / INTERIM / FINAL / SUPERSEDED`. Mirrors CSAF VEX product_status lifecycle. INTERIM is the default (runtime evidence without human review yet).
5. **`AIBOMComponent` (frozen dataclass)** — one per (verb_name, verb_version). Carries: identity, policy provenance (source, digest, rationale), observed exploitability, observed severity/band/marker_count/cef_type, observation_count, thanatosis_count, worst_level_index/name, recommendation, status, notes. `to_dict()` for JSON.
6. **`AIBOMAdvisory` (frozen dataclass)** — the self-contained advisory document with six top-level sections in CSAF-VEX order: `document` (tracking + publisher + schema + bundle_summary) + `product_tree` (components[]) + `vulnerabilities[]` (per-component finding summary) + `notes[]` (advisory-level context) + `references[]` (evidence pointers: audit log + bundle + replay) + signature placeholder.
7. **`advisory_id`** — sha256 of `{"evidence_digest": ..., "components": [...]}`. Two observers of the same bundle + audit log produce the same advisory_id. This is the substrate for downstream signature schemes (arXiv:2606.19390's "cryptographically signed" claim is signature-ready; the next-priority is `sign_advisory()` + `verify_advisory()`).
8. **`_canonical_form(payload)` + `_sha256_hex(text)`** — canonical JSON form (sorted keys, no whitespace) + sha256 hash. The canonical form is the substrate for replay verification + future signature envelopes.
9. **`_severity_to_action(band, is_thanatosis)` + `_advisory_severity(components)` + `_advisory_recommendation(severity, has_thanatosis)`** — the conservative ladders. CRITICAL+thanatosis → BLOCK; HIGH+ → REMEDIATE; MEDIUM → MONITOR; LOW/NONE → DEFER.
10. **`_category_for(components)`** — top-level category. CET → THANATOSIS; looser-than-default policy → POLICY_LOOSENING; any CEF marker → CEF_EMERGENCE; otherwise GENERIC.
11. **`_component_status(component)`** — VEX-style product_status: `known_affected` (any marker), `known_not_affected` (clean), `under_investigation` (no observations or CEF substrate unavailable).
12. **`_exploitability(component)`** — heuristic exploitability: CRITICAL+thanatosis → "critical"; HIGH+ → "high"; MEDIUM → "medium"; LOW → "low"; NONE → "none". Maps the worst observed band to a CSAF-style probability.
13. **`_evidence_digest_from_audit_log(audit_log_path, audit_log_records)`** — sha256 of either the audit log file bytes or the canonical form of supplied records. Empty string if neither is available.
14. **`_component_from_observation(verb_name, observations)`** — aggregator: per-verb observations → single AIBOMComponent. Computes worst band, worst severity, worst CEF type, worst level, thanatosis count, total marker count.
15. **`group_observations_by_verb(observations)` + `build_components(observations)`** — group the flat observation list by verb_name, then aggregate per group.
16. **`compute_advisory_id(evidence_digest, components, ...)`** — content-addressed advisory_id from evidence_digest + components (canonical form + sha256).
17. **`emit_aibom_advisory(observations, *, bundle_snapshot, audit_log_path, audit_log_records, title, publisher, status, document_category)`** — the central function. Pure: takes a flat list of CPP audit observations + optional bundle snapshot + optional audit log path/records, returns a self-contained AIBOMAdvisory dataclass. The caller chooses where to write it.
18. **`try_emit_from_bundle(bundle, outcome, *, audit_log_path, audit_log_records)`** — convenience: build the advisory from a VerbPolicyBundle + CPPOutcome. Renders the bundle via `to_dict()` or `vars()`, extracts the outcome's `observations` list, calls `emit_aibom_advisory`. This is the function the CLI uses.
19. **`write_advisory(advisory, path)`** — JSON-serialize the advisory to disk. `load_audit_log(path)` — read a JSONL audit log, with skip-malformed-line defensive behavior (records `_parse_error` alongside parsed records).
20. **`cli/aibom_review.py`** — operator-facing CLI: `python -m cli.aibom_review --audit-log cpp.jsonl [--bundle-snapshot bundle.json] [--out advisory.json] [--summary] [--table] [--silent]`. `--summary` prints a human-readable summary (advisory_id, category, severity, recommendation, total_observations, thanatosis_count, per-verb findings). `--table` prints a per-component table (component, band, severity, exploitability, recommendation). Exit code propagates the recommendation: 0 = defer/monitor, 1 = remediate, 2 = block. The CLI is the substrate for the agent gateway's "audit-trail" command.

**Conservative posture preserved**:
- The advisory is *advisory*. It carries a recommendation (defer / monitor / remediate / block); the operator (or the upstream agent gateway) decides what to do.
- No I/O at construction. `emit_aibom_advisory()` is pure: it takes a CPPOutcome / audit log + a bundle snapshot and returns a self-contained dict. The caller chooses where to write it.
- The advisory is *content-addressed*: `advisory_id` is sha256 of the canonical form of (evidence_digest, components). Two observers of the same bundle + audit log produce the same advisory_id. This is the substrate for downstream signature schemes.
- The advisory exposes the policy provenance from the bundle snapshot: which entry resolved for each verb, what its source was (SPECIFIC / WILDCARD / DEFAULT / NO_BUNDLE), and the registered rationale. The advisory is the audit trail's handoff to a human reviewer.
- No silent redaction. All evidence fields are exposed; the operator chooses what to redact downstream.
- Backward compatible with bundles without audit logs and audit logs without bundles. The advisory records what it has (`cef_substrate_available=False` if no signal; `bundle_library_id=None` if no bundle).
- The status field defaults to `INTERIM` (runtime evidence without human review yet). The operator upgrades to `FINAL` after review. The lifecycle is the CSAF-VEX lifecycle.
- The CLI's `--silent` is the *explicit opt-out* for stdout. Default behavior is human-readable summary to stdout + advisory JSON to file (or stdout if no `--out`). Exit code is the recommendation, not the band.
- The `_component_status` is the VEX lifecycle: `known_affected` (any marker), `known_not_affected` (clean), `under_investigation` (no signal). A reviewer reading the advisory sees the per-component status, not just the band.
- The load_audit_log skip-malformed-line behavior is the *defensive read*: a reviewer can inspect a partially-corrupt audit log without losing the whole file. Malformed lines are recorded as `_parse_error` records alongside the parsed ones. The advisory's evidence_digest is still computed from the parsed records (a corrupt line doesn't poison the hash).

**Test coverage**: 47/47 new tests pass in `experiments/test_aibom_advisory.py`:
- TestEnums (4): AdvisoryCategory, AdvisorySeverity, AdvisoryAction, AdvisoryStatus
- TestCanonicalForm (4): hash determinism, key order independence, payload sensitivity, sha256 format
- TestAdvisoryHelpers (5): _severity_to_action ladder, _severity_to_action thanatosis promotion, _advisory_severity takes max, _advisory_recommendation blocks on thanatosis, _exploitability uses band and CEF type
- TestComponentFromObservation (5): clean obs → clean component, CEF obs carries severity, thanatosis captured, level index set from observation, multiple observations aggregated
- TestBuildComponents (4): empty obs list, single-obs, multi-verb, worst-level selection
- TestGroupObservationsByVerb (1): groups by verb_name
- TestEmitAdvisoryBasic (5): clean probe → GENERIC, medium probe → CEF_EMERGENCE, critical probe → THANATOSIS, advisory_id deterministic, evidence_digest computed from records
- TestEmitAdvisoryBundle (4): no bundle → None library_id, bundle with library_id recorded, bundle entries surface in product_tree, bundle strict_unknown_verb recorded
...
- TestWriteAndLoad (4): write_advisory JSON round-trip, load_audit_log parses, load_audit_log skips malformed, load_audit_log missing file → empty list
- TestTryEmitFromBundle (3): happy path (bundle + outcome + audit log), missing audit log returns INTERIM status with bundle_library_id set, missing bundle returns INTERIM status with bundle_library_id None
- TestComponentStatus (3): clean component status, critical component status, medium component status

Plus 21/21 new tests pass in `experiments/test_aibom_review_cli.py`:
- TestLoadBundleSnapshot (3): happy path, missing file → None, malformed JSON → None
- TestFormatHelpers (3): _format_summary contains advisory_id, _format_advisory_table contains component, _format_summary contains thanatosis_count
- TestMainSummary (4): clean audit log summary, CEF audit log summary, thanatosis audit log blocks (exit 2), with bundle snapshot
- TestMainJsonOut (2): --out writes file, --table prints components
- TestSubprocess (3): subprocess clean (exit 0), subprocess CEF (exit 1), subprocess thanatosis (exit 2)
- TestMainArgs (2): missing --audit-log fails, --help works
- TestExitCodes (1): BLOCK exits 2, REMEDIATE exits 1, MONITOR exits 0
...
- **235/235 cross-substrate tests pass with zero regressions.**

**End-to-end smoke test** (run against the example audit log from the 2026-07-05 build):
```
$ python -m cli.aibom_review --audit-log cpp_test.jsonl --table
...
$ echo $?
2  # BLOCK (simulated_crash+critical at L7)
```
- category: `thanatosis` (the simulated_crash observation triggers THANATOSIS priority)
- severity: `critical`
- recommendation: `block`
- exit code: 2 (BLOCK) — the agent gateway's OPA evaluator would route this to human review
...
- `cli/aibom_review.py`: 216 lines (new) — operator-facing CLI with --audit-log, --bundle-snapshot, --out, --status, --title, --publisher, --summary, --table, --silent
- `experiments/test_aibom_advisory.py`: 687 lines (new) — 47 tests covering all components
- `experiments/test_aibom_review_cli.py`: 380 lines (new) — 21 tests covering CLI surface
...
### 2026-06-27 - Scheduled Run: Governor Circuit (closes next-priority from 2026-06-25)

**Status**: ✅ COMPLETE - 56/56 new tests, 216/216 CEF-adjacent, 2023/2056 total (33 pre-existing failures unrelated), zero new regressions

**Research Summary (June 26-27, 2026)**: See CURRENT_RESEARCH.md for the full report. Headlines:

- **AgentRiskBOM (arXiv:2606.21877, IEEE Cyber-AI 2026, Jun 25)** ⭐ BUILDS ON THIS — independently arrives at the same three-layer safety decomposition: capability inventory + control lineage + evidence ledger. Our `GovernorCircuit` + `ProbabilisticTripEngine` + `EvidenceLedger` is the closest functional equivalent.
- **CEF substrate completes its three legs**: measurement (`ProbabilisticTripEngine`, arXiv:2606.20510), observation (`CEFSessionDetector` + `CEFIntegrationConfig`), and now **policy** (`GovernorCircuit`). The bound is now both a measurement and a policy input.
- **Cold-engine-silent posture** (this build's signature fix): a fresh `Beta(1, 1)` engine has `trip_upper_bound = 1.0` (CRITICAL) at zero observations because the prior is uniform on [0, 1]. Without treatment, the gate would trip on the first feed — the opposite of conservative. The gate now treats cold engines (n_observations == 0) as silent (contribute bound=0 to the two-layer max). A cold engine means "no evidence", not "maximal risk".
- **Warm-up grace window** (the policy refinement): `config.warm_up_min_obs` (default 5) suppresses CLOSED -> OPEN transitions during the first N observations. The suppressed decision is recorded as a `warming_grace` transition so the audit trail is complete. Without warm-up, a gate could trip on the wide posterior quantile of one observation (Beta(2, 1) bound ≈ 0.78).
- **AgentRiskBOM distinction** "capability opacity" vs "demonstrated behavior" — the cold-engine-silent posture is the operational form. Absence of evidence is a fact about the engine's state, never conflated with absence of risk.
- **Trending repos**: Hephaestus (v0.7.23), Hermes Agent (v0.15.0), Continuum (v0.2.3), OpenLAIR/OpenSkill (Jun 2026) — all converging on the same verifier-above-stateful-memory pattern; our substrate is the closest functional equivalent.

**Build Task: Governor Circuit — stateful policy gate driven by `ProbabilisticTripEngine` bounds**

**Motivation**: every safety measurement built so far produces a sound (1 - alpha) bound on the agent's failure rate. But a measurement that nobody reads is a hidden alarm. Today's build is the operational instrument: a stateful policy gate that *acts* on the bound. This closes the 2026-06-25 next-priority item.

**Key Components**:
1. `GovernorCircuit` + `GovernorCircuitConfig` + `GateState` (CLOSED / OPEN / HALF_OPEN) + `GateAction` (ALLOW / BLOCK / PROBE) + `GateObservation` + `GateTransition` + `GovernorDecision`. Mirrors `SafetyCircuitBreaker` contract.
2. **Three-state hysteresis** with `reset_threshold < trip_threshold` invariant: CLOSED -> OPEN on `bound >= trip_threshold`, OPEN -> HALF_OPEN on `bound <= reset_threshold`, HALF_OPEN -> CLOSED on `consecutive_good >= probe_count`.
3. **Two-layer max** when two engines configured (per_output + session): the gate's bound is the *maximum* of the two engines' bounds. Gate trips when EITHER layer reports high; recovers when BOTH are low.
4. **Cold-engine-silent posture**: fresh engine with `Beta(1, 1)` prior and zero observations has `trip_upper_bound = 1.0` (CRITICAL). Gate now treats cold engines (n_observations == 0) as silent (bound=0 contribution to two-layer max). Cold engine means "no evidence", not "maximal risk".
5. **Warm-up grace**: `config.warm_up_min_obs` (default 5) suppresses CLOSED -> OPEN during the first N observations. Suppressed decision recorded as `warming_grace` transition for audit completeness.
6. **`n_observations` property** added to `ProbabilisticTripEngine` (alias for `n_total`) so the `BoundSource` protocol can express cold-vs-warmed state explicitly.
7. **Append-only transition log** with `write_audit(path)` JSONL output, idempotent on repeated calls.
8. **`decide()` is read-only** — does not transition state, does not increment `consecutive_good`. `feed(observation)` is the state-mutating call; `decide()` is the read-only "what would the gate do now" query.
...
- `reset_threshold < trip_threshold` enforced in `__post_init__`.
...
**Test Coverage**: 56/56 tests passed ✅
- TestGovernorCircuitConfig (7), TestGateEnums (2), TestGateObservation (3), TestGateTransition (3), TestGovernorDecision (2)
- TestGovernorCircuitNoEngine (5), TestGovernorCircuitSingleLayer (3), TestGovernorCircuitTwoLayerMax (3)
- TestGovernorCircuitHysteresis (6), TestGovernorCircuitAudit (4), TestGovernorCircuitConvenience (4)
- TestGovernorCircuitIntegration (3), TestGovernorCircuitConservativePosture (3)
- TestGovernorCircuitWarmUp (4), TestGovernorCircuitFeedValidation (3)

**Cross-suite regression check**: 216/216 CEF-adjacent tests pass across `test_governor_circuit` (56), `test_cef_probabilistic_verification` (47), `test_cef_substrate_integration` (44), `test_agent_loop_cef_bridge` (24), `test_cef_detector` (15), `test_cef_session` (34). Total 216/216.

**Files Changed**:
- `core/governor_circuit.py`: 1105 lines (new)
- `experiments/test_governor_circuit.py`: 865 lines (new) — 56 tests
- `core/cef_probabilistic_verification.py`: +10 lines — `n_observations` property
- `CURRENT_RESEARCH.md`: 2026-06-27 research update appended
- `BUILD_LOG_2026-06-27.md`: this entry
- `AGENTS.md`: this build log entry

**Next Priority**:
- **`assess_cef_to_breaker` session-band integration**: surface the session band alongside the per-output band in the breaker outcome when the session is in `POINT_OF_NO_RETURN`. Operator sees both layers' measurements in one place — and both layers feed the same `GovernorCircuit`.
- **CLI / dashboard for the gate**: `python -m core.governor_circuit --summary <gate_dict>` prints current state, bound, band, transition count, and audit-path JSONL line count.
- **Multi-session engine persistence** (carried from 2026-06-25): engine should be backed by append-only JSONL for fleet-scale.
