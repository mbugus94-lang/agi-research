...
def test_review_cli_rejects_tampered_composite_source_against_signed_advisory(tmp_path):
    source = {
        "trend": {
            "category": "cage1_decision_fleet_audit_trend",
            "schema_version": "1.0",
            "status": "stable",
            "points": [{"snapshot_id": "trend-only", "line_provenance": [{"source_id": "trend-invalid.jsonl", "source_line_number": 44, "status": "invalid_schema", "reason": "trend-only evidence is excluded from the canonical fleet view"}]}],
            "flagged_changes": [],
            "automatic_action_taken": False,
        },
        "fleet": {
    ...
    tampered["fleet"]["lines"][0]["decision"] = "accept"
    source_path = tmp_path / "tampered-composite.json"
    envelope_path = tmp_path / "decision.json"
    report_path = tmp_path / "decision-report.json"
    source_path.write_text(json.dumps(tampered), encoding="utf-8")
    envelope_path.write_text(envelope_to_json(envelope), encoding="utf-8")
    result = subprocess.run([
        sys.executable, "-m", "cli.cage1_review",
        "--trend-input", str(source_path),
        "--decision-envelope", str(envelope_path),
        "--decision-key-id", "k1",
        "--decision-hmac-secret", "secret",
        "--decision-now", "10",
        "--decision-report-out", str(report_path),
    ...
    assert verification["decision"] is None
    assert verification["raw_evidence_status"] == "match"
    assert "advisory_mismatch" in verification["invalid_statuses"]
    assert verification["decision_applied"] is False
    assert verification["automatic_action_taken"] is False
    saved = json.loads(report_path.read_text(encoding="utf-8"))
