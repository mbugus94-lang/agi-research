...
### Build: trend evidence-status parity

Closed the previous run's next priority:

- Trend inputs whose schema-invalid evidence is stored under `points[].line_provenance` now receive the same machine-readable evidence counts and Markdown presentation as ordinary fleet inputs.
...
- https://arxiv.org/abs/2607.07916v1
- https://arxiv.org/abs/2607.26179v1
...
- https://github.com/Nanako0129/pilotfish
- https://github.com/robbyant/lingbot-world-v2
## 2026-07-25 - Scheduled Run: Fleet-Level CAGE-1 Decision Audit Aggregation

**Status**: COMPLETE — added a verification-only fleet aggregator for decision audit JSONL files; focused fleet/consumer/signing suite passes **91/91**.
...
### Build: verification-only fleet audit aggregator
...
**Safety boundary**: verification only. The aggregator never applies `accept`, `reject`, or `defer`, never repairs or drops malformed/expired records, and hard-codes `decision_applied=False` and `automatic_action_taken=False`.
